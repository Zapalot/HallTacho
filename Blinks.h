...
public:
  ...
  void setup(){
    SingleLightEndSpeedSetting.setup(&singleLightEndSpeed,0,100000,F("single/end"),&oscServer,(int)&persistentSingleLightEndSpeed);
  };
  void printState(){
    for( int i=0;i<RL_N_CHANNELS;i++){
  ...
  void updateOuts(long speed){
    writeMovingSingleLight(speed);
  };


  void writeMovingSingleLight(long speed){
